  ...
  const drawChart = (): void => {
    ...
    const drawBoxAround = (textElement: any, factor?: number): void => {
      ...
      if (accessibleMode) {
        ...
        const newX = Number(bbox.x) + (bbox.width - newWidth) / 2
    ...
    if ((!(xAxisLabels.length > ticks) || accessibleMode) && adjustedXLabels.length > ticks) {
    ...
    svg.append('line')
      ...
      .style('stroke', accessibleMode ? 'black' : '#aeaeb4')
      .attr('stroke-width', 2.5)
    if ((!(yAxisLabels.length > ticks) || accessibleMode) && adjustedYLabels.length > ticks) {
